...
var _sendSms = _interopRequireDefault(require("../helpers/send-sms"));
...
function makeSubscribersQuery({
  ...
  async function add({
    ...
    const {
      result,
      ops
    } = await db.collection('Subscribers').insertOne(subscribers).catch(mongoError => {
  ...
  async function verify({
    ...
    if (found) {
      ...
      const found2 = await db2.collection('Subscribers').findOne({
        surname: surname
      });

      if (found2) {
        (0, _sendSms.default)("Your subscription has been submitted", "08121631789");
        return {
          status: "success",
          message: "Reord found"
        };
  ...
  function documentToSubscribers({
    ...
    return (0, _subscriber.default)({
      id,
      ...doc
    });
  }
}
//# sourceMappingURL=subscriber-query.js.map