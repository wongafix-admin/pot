...
function makeBankQuery({
  ...
  async function getBank({
    ...
    return (await db.collection('Bank').find(query).limit(Number(max)).toArray()).map(documentToBank);
  }
  ...
  async function add({
    ...
    const {
      result,
      ops
    } = await db.collection('Bank').insertOne(bank).catch(mongoError => {
  ...
  async function update({
    ...
    const {
      result
    } = await db.collection('Bank').updateOne(query, newSet, {
      upsert: true
    });
  ...
  function documentToBank({
    ...
    return (0, _bank.default)({
      id,
      ...doc
    });
  }
}
//# sourceMappingURL=bank-query.js.map