...
exports.default = makeClientQuery;
...
function makeClientQuery({
  ...
  async function getClients({
    ...
    return (await db.collection('Client').find(query).limit(Number(max)).toArray()).map(documentToClient);
  }
  ...
  async function add({
    ...
    const {
      result,
      ops
    } = await db.collection('Client').insertOne(client).catch(mongoError => {
